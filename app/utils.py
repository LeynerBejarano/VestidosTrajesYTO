...
def timeRange(horas_usadas=None):
    hora_i = time(6,0)
    hora_f = time(21,30)
    delta = timedelta(minutes=20)
    horas = []
    hora = hora_i
    if not horas_usadas or (horas_usadas and hora.strftime('%I:%M %p') not in horas_usadas):
        horas.append(hora.strftime('%I:%M %p'))
    while(hora < hora_f):
        dt = datetime.combine(date.today(), hora) + delta
        hora = dt.time()
        if not horas_usadas or (horas_usadas and hora.strftime('%I:%M %p') not in horas_usadas):
            horas.append(hora.strftime('%I:%M %p'))
    return horas
